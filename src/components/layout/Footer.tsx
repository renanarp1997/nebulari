...
export function Footer() {
  return (
    <footer className="footer-premium border-t bg-surface-dark text-white/75">
      <div className="site-container px-4 py-10 sm:px-8 sm:py-16">
        <div className="mb-8 grid gap-3 border-b border-white/12 pb-8 sm:mb-12 sm:grid-cols-3 sm:gap-4 sm:pb-10">
          {FOOTER_TRUST.map((item, i) => {
            const Icon = trustIcons[i];
            return (
              <div
                key={item.label}
                className="flex items-start gap-4 border border-white/10 bg-white/[0.03] px-5 py-4"
              >
            ...
            <div className="mt-7 flex gap-2.5">
              {socialLinks.map(({ icon: Icon, label, href }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="flex h-10 w-10 items-center justify-center border border-white/18 text-white/75 transition hover:border-white/40 hover:text-white"
                >
        ...
        <div className="mt-14 flex flex-col gap-8 border-t border-white/12 pt-10 sm:flex-row sm:items-end sm:justify-between">
          <div>
